...
if (!window.location.pathname.includes('/api')) {
    ...
    document.querySelectorAll('.view-order-btn').forEach(btn => {
        ...
        <button class="btn btn-danger" data-action="cancel">Скасувати</button>
    `;

                // Додаємо обробники для кнопок всередині модального вікна
                    ...
                    btn.addEventListener('click', async function () {
                        ...
                        try {
                            if (action === 'confirm') {
                                ...
                                await updateOrderStatus(data.order.id, 'Cancelled');
                            }
                        } catch (err) {
                ...
                // Для статусу Going показуємо кнопку "Завершити"
                actions.innerHTML = `
        <button class="btn btn-success" data-action="finish">Завершити</button>
    `;

                const finishBtn = actions.querySelector('button[data-action="finish"]');
                if (finishBtn) {
                    finishBtn.addEventListener('click', async function () {
                        try {
                            await updateOrderStatus(data.order.id, 'Shipped');
                        } catch (err) {
                            console.error('Помилка при завершенні замовлення:', err);
                            alert('Не вдалося завершити замовлення');
                        }
                    });
                }
            } else if (data.order.status === 'Shipped') {
                // Для статусу Shipped показуємо кнопку "Завершити" щоб поміняти на Completed
                actions.innerHTML = `
        <button class="btn btn-primary" data-action="complete">Завершити</button>
    `;

                const completeBtn = actions.querySelector('button[data-action="complete"]');
                if (completeBtn) {
                    completeBtn.addEventListener('click', async function () {
                        try {
                            await updateOrderStatus(data.order.id, 'Completed');
                        } catch (err) {
                            console.error('Помилка при завершенні замовлення:', err);
                            alert('Не вдалося позначити замовлення як виконане');
                        }
                    });
                }
            }

            // Якщо сервер повернув статус 'Going', показуємо 'Shipped'
            document.getElementById('orderStatus').textContent = (data.order.status === 'Going') ? 'Shipped' : data.order.status;
